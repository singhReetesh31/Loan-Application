...
public class LoanApplicationService implements ILoanApplicationService {
	...
	public LoanApplicationDto approveLoanApplication(int loanId) throws LoanApplicationNotFoundException {
		...
		dto.setId(loanApp.getId());
		dto.setBankName(loanApp.getBankName());
		dto.setMinLoanAmount(loanApp.getMinLoanAmount());
		dto.setMaxLoanAmount(loanApp.getMaxLoanAmount());
		dto.setMinInterestRate(loanApp.getMinInterestRate());
		dto.setMinCreditScore(loanApp.getMinCreditScore());
		dto.setTermLength(loanApp.getTermLength());
		dto.setProcessingFee(loanApp.getProcessingFee());
		dto.setRating(loanApp.getRating());
		dto.setStatus(loanApp.getStatus());
		return dto;
	}
